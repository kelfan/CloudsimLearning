 ...
 */
public class MergeDatacenter extends Datacenter {
        /**
	...
	public Map<Integer, Integer> HostToSwitchid;
         ...
         */
	public Map<Integer, MergeSwitch> Switchlist;
	...
	public MergeDatacenter(
		...
		Switchlist = new HashMap<Integer, MergeSwitch>();
	}
	 ...
	 */
	public Map<Integer, MergeSwitch> getEdgeSwitch() {
		Map<Integer, MergeSwitch> edgeswitch = new HashMap<Integer, MergeSwitch>();
		for (Entry<Integer, MergeSwitch> es : Switchlist.entrySet()) {
			if (es.getValue().level == MergeConstants.EDGE_LEVEL) {
	...
	public boolean processVmCreateNetwork(Vm vm) {
		...
		if (result) {
			VmToSwitchid.put(vm.getId(), ((MergeHost) vm.getHost()).sw.getId());
