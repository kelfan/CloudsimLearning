 ...
 */
public class MergeNetworkCloudletSpaceSharedScheduler extends CloudletScheduler {
	/** The current CPUs. */
	 ...
	 * @post $none
	 */
	public MergeNetworkCloudletSpaceSharedScheduler() {
		super();
		...
		usedPes = 0;
		currentCpus = 0;
		pkttosend = new HashMap<Integer, List<MergeHostPacket>>();
		pktrecv = new HashMap<Integer, List<MergeHostPacket>>();
	}
	...
	public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
		...
		for (ResCloudlet rcl : getCloudletExecList()) { // each machine in the
			...
			MergeCloudlet cl = (MergeCloudlet) rcl.getCloudlet();
			...
			if ((cl.currStagenum != -1)) {
				if (cl.currStagenum == MergeConstants.FINISH) {
					break;
				}
				MergeTaskStage st = cl.stages.get(cl.currStagenum);
				if (st.type == MergeConstants.EXECUTION) {
				...
				if (st.type == MergeConstants.WAIT_RECV) {
					List<MergeHostPacket> pktlist = pktrecv.get(st.peer);
					List<MergeHostPacket> pkttoremove = new ArrayList<MergeHostPacket>();
					if (pktlist != null) {
						Iterator<MergeHostPacket> it = pktlist.iterator();
						MergeHostPacket pkt = null;
						if (it.hasNext()) {
							pkt = it.next();
				...
				if (cl.stages.get(0).type == MergeConstants.EXECUTION) {
					MergeNetDatacenterBroker.linkDC.schedule(
							MergeNetDatacenterBroker.linkDC.getId(),
		...
		for (ResCloudlet rcl : getCloudletExecList()) {
			// rounding issue...
			if (((MergeCloudlet) (rcl.getCloudlet())).currStagenum == MergeConstants.FINISH) {
				// stage is changed and packet to send
				((MergeCloudlet) (rcl.getCloudlet())).finishtime = CloudSim.clock();
				toRemove.add(rcl);
				cloudletFinish(rcl);
				finished++;
			}
	...
	private void changetonextstage(MergeCloudlet cl, MergeTaskStage st) {
		cl.timespentInStage = 0;
		cl.timetostartStage = CloudSim.clock();
		int currstage = cl.currStagenum;
		if (currstage >= (cl.stages.size() - 1)) {
			cl.currStagenum = MergeConstants.FINISH;
		} else {
			cl.currStagenum = currstage + 1;
			int i = 0;
			for (i = cl.currStagenum; i < cl.stages.size(); i++) {
				if (cl.stages.get(i).type == MergeConstants.WAIT_SEND) {
					MergeHostPacket pkt = new MergeHostPacket(
							cl.getVmId(),
					...
					List<MergeHostPacket> pktlist = pkttosend.get(cl.getVmId());
					if (pktlist == null) {
			...
			if (i == cl.stages.size()) {
				...
				cl.currStagenum = i;
				if (cl.stages.get(i).type == MergeConstants.EXECUTION) {
					MergeNetDatacenterBroker.linkDC.schedule(
							MergeNetDatacenterBroker.linkDC.getId(),
