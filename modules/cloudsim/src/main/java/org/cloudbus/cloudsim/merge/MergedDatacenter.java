...
/**
 * MergedDatacenter class is a {@link Datacenter} whose hostList are virtualized and networked. It contains
 ...
 */
public class MergedDatacenter extends Datacenter {
	...
	/**
	 * Instantiates a new MergedDatacenter object.
	 * 
	...
	/**
	 * Creates the given VM within the MergedDatacenter.
	...
	public boolean processVmCreateNetwork(Vm vm) {
		...
		if (result) {
			VmToSwitchid.put(vm.getId(), ((MergedHost) vm.getHost()).sw.getId());
	...
	protected void processCloudletSubmitPower(SimEvent ev, boolean ack) {
		super.processCloudletSubmit(ev, ack);
		setCloudletSubmitted(CloudSim.clock());
	}


	protected void processCloudletSubmitNetwork(SimEvent ev, boolean ack) {
		...
		try {
			...
			Log.printLine(getName() + ".processCloudletSubmitNetwork(): " + "ClassCastException error.");
			c.printStackTrace();
		} catch (Exception e) {
			Log.printLine(getName() + ".processCloudletSubmitNetwork(): " + "Exception error.");
			e.printStackTrace();
		}
