 ...
 */
public class MergeNetDatacenterBroker extends SimEntity {
	...
	/** The list of submitted {@link MergeCloudlet MergeCloudlets}. */
	private List<? extends MergeCloudlet> cloudletList;

	/** The list of submitted {@link MergeAppCloudlet MergeAppCloudlets}. */
	private List<? extends MergeAppCloudlet> appCloudletList;
	...
	public static MergeDatacenter linkDC;
	 ...
	 * @post $none
	 */
	public MergeNetDatacenterBroker(String name) throws Exception {
		super(name);

		setVmList(new ArrayList<MergeVm>());
		setVmsCreatedList(new ArrayList<MergeVm>());
		setCloudletList(new ArrayList<MergeCloudlet>());
		setAppCloudletList(new ArrayList<MergeAppCloudlet>());
	 ...
	 * @post $none
	 */
	public void submitCloudletList(List<? extends MergeCloudlet> list) {
		getCloudletList().addAll(list);
	}

	public static void setLinkDC(MergeDatacenter alinkDC) {
		linkDC = alinkDC;
	}
	...
	protected void processCloudletReturn(SimEvent ev) {
		...
		if (getCloudletList().size() == 0 && cloudletsSubmitted == 0 && MergeConstants.iteration > 10) {
	...
	protected void createVmsInDatacenterBase(int datacenterId) {
		...
		for (int i = 0; i < 100; i++) {
			this.getAppCloudletList().add(
					new MergeWorkflowApp(MergeAppCloudlet.APP_Workflow, MergeConstants.currentAppId, 0, 0, getId()));
			MergeConstants.currentAppId++;

		}
		int k = 0;

		// schedule the application on VMs
		for (MergeAppCloudlet app : this.getAppCloudletList()) {
		...
		setAppCloudletList(new ArrayList<MergeAppCloudlet>());
		if (MergeConstants.iteration < 10) {

			MergeConstants.iteration++;
			this.schedule(getId(), MergeConstants.nexttime, CloudSimTags.NextCycle);
		}
	...
	private void CreateVMs(int datacenterId) {
		// two VMs per host
		int numVM = linkDC.getHostList().size() * MergeConstants.maxhostVM;
		for (int i = 0; i < numVM; i++) {
			...
			long bw = 1000;
			int pesNumber = MergeConstants.HOST_PEs / MergeConstants.maxhostVM;
			String vmm = "Xen"; // VMM name

			// create VM
			MergeVm vm = new MergeVm(
					vmid,
					getId(),
					mips,
					pesNumber,
					...
					size,
					vmm,
					new MergeNetworkCloudletSpaceSharedScheduler());
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <T extends MergeCloudlet> List<T> getCloudletList() {
	 ...
	 */
	protected <T extends MergeCloudlet> void setCloudletList(List<T> cloudletList) {
		this.cloudletList = cloudletList;
	}

	@SuppressWarnings("unchecked")
	public <T extends MergeAppCloudlet> List<T> getAppCloudletList() {
		return (List<T>) appCloudletList;
	}

	public <T extends MergeAppCloudlet> void setAppCloudletList(List<T> appCloudletList) {
