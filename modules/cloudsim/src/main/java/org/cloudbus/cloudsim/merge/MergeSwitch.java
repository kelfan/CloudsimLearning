 ...
 */
public class MergeSwitch extends SimEntity {
	...
	public int datacenterid;
         ...
         */
	public Map<Integer, List<MergeNetworkPacket>> uplinkswitchpktlist;
         ...
         * value is the packets sent to that switch.
         */
	public Map<Integer, List<MergeNetworkPacket>> downlinkswitchpktlist;

        /**
         * Map of hosts connected to the switch, where each key is the host ID
         * and the corresponding value is the host itself.
         */
	public Map<Integer, MergeHost> hostlist;

        /**
         * List of uplink switches.
         */
	public List<MergeSwitch> uplinkswitches;

        /**
         * List of downlink switches.
         */
	public List<MergeSwitch> downlinkswitches;
         ...
         */
	public Map<Integer, List<MergeNetworkPacket>> packetTohost;
	...
	public double numport;
         ...
         * @todo It doesn't appear to be used
         */
	public MergeDatacenter dc;

	/** Something is running on these hosts.
         * @todo The attribute is only used at the TestExample class. */
	public SortedMap<Double, List<MergeHost>> fintimelistHost = new TreeMap<Double, List<MergeHost>>();

	/** Something is running on these hosts.
         * @todo The attribute doesn't appear to be used */
	public SortedMap<Double, List<MergeVm>> fintimelistVM = new TreeMap<Double, List<MergeVm>>();

        /**
         * List of  received packets.
         */
	public ArrayList<MergeNetworkPacket> pktlist = new ArrayList<MergeNetworkPacket>();
	...
	public List<Vm> BagofTaskVm = new ArrayList<Vm>();
         ...
         * constants such as {@link MergeConstants#SwitchingDelayRoot},
	...
	public double switching_delay;
         ...
         */
	public Map<Integer, MergeVm> Vmlist = new HashMap<Integer, MergeVm>();

	public MergeSwitch(String name, int level, MergeDatacenter dc) {
		super(name);
		this.level = level;
		this.dc = dc;
	}
	...
	protected void processhostpacket(SimEvent ev) {
        ...
        MergeHost hs = hostlist.get(hspkt.recieverhostid);
	...
	protected void processpacket_up(SimEvent ev) {
		...
		if (level == MergeConstants.EDGE_LEVEL) {
			...
			int hostid = dc.VmtoHostlist.get(recvVMid);
            MergeHost hs = hostlist.get(hostid);
			hspkt.recieverhostid = hostid;
			if (hs != null) {
		...
		if (level == MergeConstants.ROOT_LEVEL) {
			...
			;
			for (MergeSwitch sw : downlinkswitches) {
				for (MergeSwitch edge : sw.downlinkswitches) {
			...
			if (aggSwtichid < 0) {
				...
				List<MergeNetworkPacket> pktlist = downlinkswitchpktlist.get(aggSwtichid);
				if (pktlist == null) {
					pktlist = new ArrayList<MergeNetworkPacket>();
	...
	private void registerHost(SimEvent ev) {
        MergeHost hs = (MergeHost) ev.getData();
	...
	protected void processpacket(SimEvent ev) {
		...
		pktlist.add((MergeNetworkPacket) ev.getData());
	...
	protected void processpacketforward(SimEvent ev) {
		...
		if (downlinkswitchpktlist != null) {
			for (Entry<Integer, List<MergeNetworkPacket>> es : downlinkswitchpktlist.entrySet()) {
				int tosend = es.getKey();
				List<MergeNetworkPacket> hspktlist = es.getValue();
				if (!hspktlist.isEmpty()) {
					double avband = downlinkbandwidth / hspktlist.size();
					Iterator<MergeNetworkPacket> it = hspktlist.iterator();
					while (it.hasNext()) {
						MergeNetworkPacket hspkt = it.next();
		...
		if (uplinkswitchpktlist != null) {
			for (Entry<Integer, List<MergeNetworkPacket>> es : uplinkswitchpktlist.entrySet()) {
				int tosend = es.getKey();
				List<MergeNetworkPacket> hspktlist = es.getValue();
				if (!hspktlist.isEmpty()) {
		...
		if (packetTohost != null) {
			for (Entry<Integer, List<MergeNetworkPacket>> es : packetTohost.entrySet()) {
				List<MergeNetworkPacket> hspktlist = es.getValue();
				if (!hspktlist.isEmpty()) {
					double avband = downlinkbandwidth / hspktlist.size();
					Iterator<MergeNetworkPacket> it = hspktlist.iterator();
         ...
         */
	protected MergeHost getHostwithVM(int vmid) {
		for (Entry<Integer, MergeHost> es : hostlist.entrySet()) {
         ...
         */
	protected List<MergeVm> getfreeVmlist(int numVMReq) {
		List<MergeVm> freehostls = new ArrayList<MergeVm>();
		for (Entry<Integer, MergeVm> et : Vmlist.entrySet()) {
         ...
         */
	protected List<MergeHost> getfreehostlist(int numhost) {
		List<MergeHost> freehostls = new ArrayList<MergeHost>();
		for (Entry<Integer, MergeHost> et : hostlist.entrySet()) {
